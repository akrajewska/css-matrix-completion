...
print(f"rmse cscmc {np.linalg.norm(M_filled[:, cols_idx] - M[:, cols_idx]) / np.linalg.norm(M[:, cols_idx])}")
...
print(f"rmse_omega nn {np.linalg.norm(M_filled[missing_mask] - M[missing_mask]) / np.linalg.norm(M[missing_mask])}")
